...
## 总结
- 基本结构：
  - 首先在服务端生成html，然后发回给客户端，并挂上一个client.js的script给这个html
  - 客户端收到html会运行client.js，这script内部调用ReactDOM.hydrate()给客户端注水，激活所有浏览器相关的功能，比如event handler
- 处理路由：用到一个shared的路由config
  - 服务端服务器设置，不管什么path，都交给renderer处理。renderer渲染路由config，将app包进StaticRouter路由并设置location跳到相应界面。
  - 客户端类似，也是在client.js里将app包进BrowserRouter路由就行了
- 处理redux
